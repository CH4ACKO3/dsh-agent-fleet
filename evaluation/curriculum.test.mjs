...
async function evaluatedFixture(f) {
  ...
  const execute = async argv => {
    ...
    writeFileSync(join(job.output, 'results', 'task.md'), job.split === 'train' ? 'normalized training prompt' : 'RAW-SEALED-PROMPT')
...
test('only allowlisted training feedback is exported; restart reuses completed episodes', async () => {
  ...
  const evidenceDirectories = readdirSync(join(exportRoot, 'episodes'))
  const evidence = evidenceDirectories.map(directory => readFileSync(join(exportRoot, 'episodes', directory, 'answer.txt'), 'utf8'))
  assert.deepEqual(evidence, ['training solution', 'training solution'])
  assert.deepEqual(evidenceDirectories.map(directory => readFileSync(join(exportRoot, 'episodes', directory, 'task.md'), 'utf8')), ['normalized training prompt', 'normalized training prompt'])
