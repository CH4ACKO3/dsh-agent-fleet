...
- `acceptance`: Vote owned by the adversarial reproducer and dependent on `package`. Decision statement: "The final bundle contains a valid explicit construction and an independent global lower bound meeting at the same tile count, with runnable commands and hashes."
...
Checkpoint runnable source and current status before every solver call expected to exceed five minutes. Long-running commands must write logs incrementally. All algorithms must accept configurable time limits and seeds rather than embedding host-specific assumptions.
...
- `e2e-output/matilda-eval/final/result.json`: accepted/rejected status, certified lower/upper bounds, claimed optimum only when accepted, solver versions, seeds, elapsed times, artifact paths, and observable collaboration-cost counters. Include member model steps, tool calls, aborted tool calls, Channel/private messages, Reply Tasks, idle recoveries, and prompt/output tokens when the Fleet trace exposes them; use `null` plus a reason when a counter is unavailable.
- `e2e-output/matilda-eval/final/reproduce.sh`: non-interactive replay from `/workspace`.
- `e2e-output/matilda-eval/final/manifest.sha256`: SHA-256 hashes of every terminal evidence artifact.
