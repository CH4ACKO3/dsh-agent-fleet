...
describe('Fleet Team activity memory', () => {
  ...
  it('does not route assistant recall through member-only visibility', async () => {
    const { algorithm, searchTeamHistory } = fixture()
    const assistantMeta = {
      ...request.meta,
      attributes: { sessionId: 'session-assistant' },
    }
    expect(algorithm.filter({ operation: 'retrieve', meta: assistantMeta })).toBe(false)
    await expect(algorithm.retrieve?.(
      { ...request, meta: assistantMeta },
      { effort: 'medium' },
    )).resolves.toMatchObject({ handled: false, items: [] })
    expect(searchTeamHistory).not.toHaveBeenCalled()
  })
})
