...
代码跨度、围栏代码块、Markdown 引用行和转义的 `\@name` 只作为参考正文，不解析为新的
回复要求。结构化 `mentions` 仍明确创建回复义务。引用旧协作日志或工具示例时应使用这些
格式；普通状态报告请使用不带 `@` 的成员姓名。

`fleet_inbox read` 聚合读取时按最早未读顺序消耗字符预算，长消息从已读位置继续，避免
持续到来的频道动态挤掉早先依赖；指定会话的历史浏览继续保留原有分页行为。
