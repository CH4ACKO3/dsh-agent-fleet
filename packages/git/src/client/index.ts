...
function useGitJoyride(root: string | undefined, teamId: string, state: GitViewState): void {
  const joyride = useJoyride()
  useEffect(() => {
    ...
    const dispose = [
      joyride.register({
        ...
        perform: async input => {
          ...
          await openDiff(root, teamId, { path, staged })
          return { path, staged }
        },
      }),
      joyride.register({
  ...
  }, [joyride, root, state.snapshot, teamId])
}
