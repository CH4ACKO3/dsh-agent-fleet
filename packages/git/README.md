...
Without this package, Fleet does not intercept Git: members use ordinary Git commands and Team rules like other DSH Agents. With it installed, Fleet can check an intended terminal operation against the member's native DSH Session workspace, repository, branch binding, and paths, and can explicitly create a member worktree. The UI remains a supervision surface for repository state and history.
