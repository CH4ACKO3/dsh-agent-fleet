...
describe('self-evolving Team template', () => {
  ...
  it('carries observed problems and role improvements across generations', () => {
    ...
    const sourceCompose = readFileSync(resolve('examples/self-evolving-team/compose.source.yaml'), 'utf8')
    const supervisor = readFileSync(resolve('examples/self-evolving-team/scripts/supervisor.mjs'), 'utf8')
    const team = JSON.parse(readFileSync(resolve('examples/self-evolving-team/team.local.json'), 'utf8'))
    ...
    expect(compose).not.toContain('model-egress')
    expect(compose).not.toContain('PNPM_CONFIG_OFFLINE')
    expect(compose).not.toContain('NPM_CONFIG_OFFLINE')
    expect(compose).toContain('127.0.0.1:${SELF_EVOLVE_HOST_PORT:?SELF_EVOLVE_HOST_PORT is required}:3081')
    expect(sourceCompose).toContain('${FLEET_PACKAGE_PATH:?FLEET_PACKAGE_PATH must point to a built Fleet tgz}')
    expect(sourceCompose).not.toContain('${SELF_EVOLVE_WORKSPACE}/.self-evolve/dsh-agent-fleet.tgz')
