...
describe('Fleet automatic bootstrap', () => {
  ...
  it('relays actionable generation events once and advances past informational events', async () => {
    ...
    expect(relayed.content[0].text).toContain('不存在另一个 generation.ready')
    expect(relayed.content[0].text).toContain('通过 Git 从该提交读取 evidence')
    expect(relayed.content[0].text).toContain('命令和范围相同时比较')
