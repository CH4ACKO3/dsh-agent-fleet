...
test('installs and activates required Harmony Providers', () => {
  ...
  expect(manifest.dsh.harmony.requires).toEqual({
    'the-binding-of-dsh': '>=0.1.5 <0.2.0',
  })
  expect(manifest.dsh.plugin.compatibility.requires).toEqual({
    'dsh-harmony': '>=0.8.6 <0.9.0',
    'the-binding-of-dsh': '>=0.1.5 <0.2.0',
  })
  expect(manifest.dependencies['dsh-harmony']).toBe('>=0.8.6 <0.9.0')
  expect(manifest.dependencies['the-binding-of-dsh']).toBe('>=0.1.5 <0.2.0')
