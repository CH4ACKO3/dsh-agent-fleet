...
export class FleetRunService {
  ...
  private bindAssistantInput(agent: Agent): void {
    ...
    context.on('system-prompt/assemble', async (_assembly, _assembleContext, next) => {
      ...
      const request = [
        ...
        'When a multi-step request becomes a formal Goal, preserve every required terminal action and externally visible side effect in that Goal. Completion of a preparatory step is not completion of the original request; keep or create the smallest durable follow-up instead of silently dropping the remaining action.',
  ...
  private finishWork(
    ...
    if (record.continuous === true
      && record.assistants.some(assistant => assistant.sessionId === String(caller.id))) {
      throw new Error('the assistant for a continuous Fleet Team cannot close its own Team; use an external operator or host lifecycle transition')
    }
        ...
        text: this.userLocale === 'zh-CN'
          ? hasGoal
            ...
            : record.continuous === true
              ? '团队成员已全部空闲，但当前 Team 配置为持续运行。请立即创建或恢复下一项 Goal。若确实在等待外部代际事件，必须先确认外部控制请求已成功；外部事件中继会自动建立持久等待，不要用 Fleet Goal、暂停或关闭来冒充。'
          ...
          : hasGoal
            ...
            : record.continuous === true
              ? 'Every formal Team member is idle, but this Team is configured for continuous operation. Create or resume the next Goal now. If an external lifecycle event is genuinely pending, first confirm that its control request succeeded; the external relay arms the durable wait automatically, so do not imitate it with a Fleet Goal, pause, or close.'
...
export function installRunTools(
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      instructions: { type: 'string', description: 'For continue, concrete work for a new formal-member Goal. Preserve every requested terminal action and externally visible side effect, not only its preparatory steps.' },
      ...
      stages: {
        ...
        items: {
          type: 'object', additionalProperties: false, properties: {
            ...
            owners: { type: 'array', required: true, items: { type: 'string' }, description: 'For a Goal, use exactly one accountable owner. Represent collaborators and reviewers as separate dependent Goals; multiple Goal owners are all required and any block terminates the dependent branch. For a Vote, list all required voters.' },
