...
export const FLEET_TOOL_CATALOG: readonly FleetToolCatalogEntry[] = [
  ...
  { name: 'fleet_user_task', group: 'core', family: 'assistant-control', source: 'host', description: 'Inspect, delegate, explicitly take over, report, or block the assistant persistent foreground Interaction Task.', keywords: 'user interaction foreground continue delegate take over report block task 用户 交互 前台 委派 接管 汇报 阻塞 任务', aliases: ['用户任务', '汇报结果', '继续团队工作', '助理接管'], actions: ['status', 'continue', 'take_over', 'report', 'block'] },
  ...
  { name: 'fleet_goal', group: 'tasks', family: 'planning', source: 'namespace', namespace: 'task', description: 'Create or inspect leaf Goals, submit completed work results, or report an external blocker; parent outcomes are decided by their reconciler.', keywords: 'goal owner complete block intent result 目标 负责人 完成 阻塞 意图 结果', aliases: ['我的目标', '完成目标', '目标受阻'], actions: ['list', 'get', 'create', 'complete', 'block'], privilegedActions: { create: 'task.create' } },
  ...
  { name: 'fleet_reconcile', group: 'tasks', family: 'planning', source: 'namespace', namespace: 'task', description: 'Inspect or atomically resolve a reserved ReconcileAttempt; task-attempt notices are already claimed, while claim is only for ready Tasks found through list.', keywords: 'reconcile attempt trigger stable state resolve timeout claimed ready 调和 尝试 触发器 状态 超时 已领取 就绪', aliases: ['处理调和', '任务状态决议'], actions: ['list', 'get', 'claim', 'resolve'], privilegedActions: { list: 'task.reconcile', get: 'task.reconcile', claim: 'task.reconcile', resolve: 'task.reconcile' } },
