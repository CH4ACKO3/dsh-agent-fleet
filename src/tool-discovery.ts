...
const FLEET_TOOL_CATALOG: readonly FleetToolCatalogEntry[] = [
  ...
  { name: 'fleet_messages', group: 'messages', description: 'Progressively read, search, continue, react to, or pin messages.', keywords: 'message history inbox read search chunk text unread progress 消息 历史 收件箱 搜索 分段 续读 未读 已读', actions: ['read', 'search', 'inbox', 'react', 'reactions', 'pin', 'unpin', 'pins', 'text'] },
  ...
  { name: 'fleet_progress', group: 'status', description: 'Read a reachable member current runtime state and bounded recent Session progress.', keywords: 'member progress latest session status inspect 成员 进度 最新 会话', actions: ['read'] },
  ...
  { name: 'fleet_shared', group: 'resources', description: 'List, read, update, and delete files in the Team resource directory.', keywords: 'team resource shared file delete remove 团队 资源 共享 文件 删除', actions: ['list', 'read', 'write', 'delete'], privilegedActions: { write: 'resource.write', delete: 'resource.write' } },
