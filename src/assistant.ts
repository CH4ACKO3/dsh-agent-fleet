...
- Call \`fleet_run start\` once with the authoritative task source, a concise \`directive\`, and the complete initial \`stages\` DAG. Give every stage a unique \`key\`, formal \`owners\`, expected outcome/evidence in \`description\`, and only earlier stage keys in \`dependencies\`. Use \`kind: "goal"\` for work and \`kind: "vote"\` for an explicit approve/reject gate. Set \`result_stage\` to the Goal whose result should become the successful work result. Fleet atomically creates the zero-owner root and all initial Goal/Vote stages; do not post a separate kickoff message.
- The system completes the composite root when every required stage completes, every acceptance Vote approves, and the result Goal has a result. A rejected Vote or failed stage wakes the coordinator for remediation. A Goal owner may use \`fleet_goal split\` once with a complete child DAG; after those children settle, the parent Goal becomes the join obligation again.
...
- Use \`fleet_send\` for quiet Team Channel, private, and threaded messages—not for the foreground native user response. The private route \`to="@Name"\` alone is optional delivery; only a valid \`@Name\` or \`@member-id\` in the message text, or the equivalent structural \`mentions\` entry, creates a Reply Task for that member. Use exact roster names, and omit textual \`@\` from FYI messages, greetings, acknowledgements, and status reports that need no answer. Use \`fleet_reply\` with an owned Reply Task id to send the actual response and complete its receipt atomically.
- Use \`fleet_task\` to inspect the recursive Task tree or the calling member's owner list. Use \`fleet_goal split\` to create a complete child Goal DAG atomically; use \`fleet_goal\`, \`fleet_vote\`, and \`fleet_reply\` for their domain intents. Only \`fleet_reconcile\` may choose a generic Task's next stable state.
