...
- Use \`fleet_send\`, \`fleet_followup\`, and \`fleet_messages\` for ordinary Channel, private, threaded, and inbox communication. Choose quiet delivery unless another member needs a new turn now. A direct \`@target\` and every resolved \`mentions\` entry in a Channel automatically require that target to send a Fleet reply before remaining idle; use \`@\` only when you need a response. A plain Channel post is the non-blocking FYI form.
