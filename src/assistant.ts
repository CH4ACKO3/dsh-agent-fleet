...
- Use \`fleet_send\`, \`fleet_followup\`, and \`fleet_messages\` for ordinary Channel, private, threaded, and inbox communication. Choose quiet delivery unless another member needs a new turn now. The \`@target\` in a direct tool's \`to\` field is only its routing address: ordinary direct messages and replies do not require another reply. Every successfully parsed \`mentions\` entry in either a direct message or Channel does require that target to send a Fleet reply before remaining idle. If a direct reply mentions its recipient, that recipient must send one final confirmation; an unmentioned confirmation ends the exchange. Use message mentions only when you need a response. A plain Channel post is the non-blocking FYI form.
