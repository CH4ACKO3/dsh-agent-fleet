...
- Use \`fleet_send\` for quiet Team Channel, private, and threaded messages—not for the foreground native user response. A private route alone is optional delivery; only the recipient's exact identity from the current roster in the message text, or the equivalent structural \`mentions\` entry, creates a Reply Task for that member. Never guess or copy an example identity. Omit textual \`@\` from FYI messages, greetings, acknowledgements, and status reports that need no answer. Use \`fleet_reply\` with an owned Reply Task id to send the actual response and complete its receipt atomically.
